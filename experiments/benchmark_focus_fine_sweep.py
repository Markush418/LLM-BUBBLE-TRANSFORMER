"""
Focus Bubble Fine Epsilon Sweep
================================
Sweep fino de epsilon en L12 para encontrar el minimo PPL.
Tambien prueba sin Sinkhorn (softmax puro) como baseline.
"""
...
def main():
    ...
    print("=" * 70)
    print("  FOCUS BUBBLE - FINE EPSILON SWEEP")
    print("=" * 70)
    ...
    # Epsilon sweep en L12
    epsilons = [0.001, 0.005, 0.01, 0.02, 0.03, 0.05, 0.07, 0.1, 0.15, 0.2, 0.3, 0.5, 1.0]
    layer = 12
    
    results = {"baseline_ppl": ppl_base, "gate_max": gate_max, "configs": []}
    
    print(f"\nSweeping epsilon on L{layer}...")
    print(f"{'Eps':>8} {'PPL':>10} {'Delta%':>8} {'Gate':>6}")
    print("-" * 40)
    
    for eps in epsilons:
        orig_attn = model.model.layers[layer].self_attn
        wrapper = Qwen3FocusBubbleWrapper(
            original_attn=orig_attn, epsilon=eps, tau_iters=5,
        ...
        print(f"{eps:8.3f} {ppl:10.3f} {delta:+7.2f}% {gate:>6}")
        
        results["configs"].append({
            "layer": layer, "epsilon": eps, "ppl": ppl,
            "delta_pct": delta, "gate": gate,
        })
        
        model.model.layers[layer].self_attn = orig_attn
    
    # Tambien probar tau_iters sweep con mejor epsilon
    best_eps = min(results["configs"], key=lambda c: c["ppl"])["epsilon"]
    print(f"\nBest epsilon: {best_eps}")
    
    tau_values = [1, 2, 3, 5, 7, 10]
    print(f"\nSweeping tau_iters with eps={best_eps} on L{layer}...")
    print(f"{'Tau':>8} {'PPL':>10} {'Delta%':>8} {'Gate':>6}")
    print("-" * 40)
    
    for tau in tau_values:
        orig_attn = model.model.layers[layer].self_attn
        wrapper = Qwen3FocusBubbleWrapper(
            original_attn=orig_attn, epsilon=best_eps, tau_iters=tau,
            use_psi=True, use_delta=False, lam=0.0,
        ).cuda()
        model.model.layers[layer].self_attn = wrapper
        ...
        print(f"{tau:8d} {ppl:10.3f} {delta:+7.2f}% {gate:>6}")
        
        results["configs"].append({
            "layer": layer, "epsilon": best_eps, "tau_iters": tau,
            "ppl": ppl, "delta_pct": delta, "gate": gate,
        })
        
        model.model.layers[layer].self_attn = orig_attn
    
    # Probar otras capas con mejor epsilon+tau
    print(f"\nSweeping layers with eps={best_eps}...")
    print(f"{'Layer':>8} {'PPL':>10} {'Delta%':>8} {'Gate':>6}")
    print("-" * 40)
    
    for layer in [3, 5, 7, 9, 10, 11, 12, 15, 19, 23]:
        orig_attn = model.model.layers[layer].self_attn
        wrapper = Qwen3FocusBubbleWrapper(
            original_attn=orig_attn, epsilon=best_eps, tau_iters=5,
            use_psi=True, use_delta=False, lam=0.0,
        ).cuda()
        model.model.layers[layer].self_attn = wrapper
        ...
        print(f"{'L'+str(layer):>8} {ppl:10.3f} {delta:+7.2f}% {gate:>6}")
        
        results["configs"].append({
            "layer": layer, "epsilon": best_eps, "tau_iters": 5,
            "ppl": ppl, "delta_pct": delta, "gate": gate,
        })
    ...
    out_file = out_dir / "focus_fine_sweep.json"
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2)
    
    print(f"\n{'='*70}")
    print(f"  BEST: L{best['layer']} eps={best['epsilon']} tau={best.get('tau_iters',5)}")
